...
--
-- This defines a SILC dataset household, with some additional variables for equivalence scales
-- and rankings.
--
-- TODO/FIXME
-- It renames all the SILC variables to what I thought were more intuitive 
-- names but in retrospect this was a mistake.
-- 
package EU.BE.Household is
   ...
   type Person_Rec is tagged record
      ...
      idorighh : Integer; -- (idorighh) : 
      idorigperson : Integer; -- (idorigperson) : 
      ...
      labour_market_in_work_work_history_length_of_time_in_months : Amount; -- (liwwh) : In work : work history (length of time in months)
      ...
      in_kind_housing_owned_occupied : Amount; -- (khooo) : Housing : owned-occupied
      ...
      benefit_early_retirement_early_retirement_pension_months_per_year : Amount; -- (byrmy) : Early retirement	early retirement pension - months per year
      ...
      expenditure_housing_cost_mortgage_payment_interest_and_capital_mortgage_capital : Amount; -- (xhcmomc) : Housing cost : mortgage payment (interest+capital) : mortgage capital
      assets_mortgage_year_of_loan : Amount; -- (amoyl) : Mortgage : year of loan
      assets_mortgage_loan_value : Amount; -- (amolv) : Mortgage : loan value
      
      -- END AUTOGENERATED --
      -- assets_financial_capital : Amount := 0.0; -- (afc) : Financial capital
      responsible_for_housing_costs : Boolean := False;
      -- created variables
      -- Also, household_equivalent_income and personal_equivalent_income are the same thing
      -- 
      equivalent_income_quintile : Quintiles := Quintiles'First;
      equivalent_income_decile   : Deciles := Deciles'First;
      
      net_income_rank            : Positive := Positive'First;
      equivalent_income_rank     : Positive := Positive'First;

      net_income                 : Amount := 0.0;
      equivalent_income          : Amount := 0.0;
   ...
   type Household_Data is tagged record
      assets_main_residence_number_of_rooms : Integer := 0; -- (amrrm) : Main residence : number of rooms
      assets_main_residence_tenure : Tenure_Type := Tenure_Type'First; -- (amrtn) : Main residence : tenure
      
      expenditure_housing_cost : Amount := 0.0; -- (xhc) : Housing cost
      expenditure_housing_cost_rent : Amount := 0.0; -- (xhcrt) : Housing cost : rent
      expenditure_housing_cost_mortgage_payment_interest_and_capital_mortgage_interest : Amount := 0.0; -- (xhcmomi) : Housing cost : mortgage payment (interest+capital) : mortgage interest
      num_people : Person_Count := 0;
      first_person_record : Positive;
      
      --
      -- derived variables
      -- FIXME these need incorporated fully in .Web_IO
      --
      net_income_rank            : Positive := Positive'First;
      equivalent_income_rank     : Positive := Positive'First;
      net_income                 : Amount := 0.0;
      ...
      equivalent_income_quintile : Quintiles := Quintiles'First;
      equivalent_income_decile   : Deciles := Deciles'First;
      net_income_quintile : Quintiles := Quintiles'First;
      net_income_decile   : Deciles := Deciles'First;

   end record;
   
   type Household_Rec is new Household_Data with record
      people : Person_Array;
   end record;
   
   function Oldest_Person( hh : Household_Rec ) return Person_Range;
   function Head_Of_Household( hh : Household_Rec ) return Person_Range;
   --
   -- Or 0 if no spouse
   --
   function Spouse_Of( hh : Household_Rec; person_num : Person_Range ) return Person_Count;
   
   
   function Get_Equivalence_Scale( hh : Household_Rec ) return Amount;
