...
package body Callbacks.Mefisto is
   ...
   begin
      ...
      -- if( run_state.phase < mefisto_complete )then
         ...
         -- end if;
      -- end if;
      run_state := Run_State_Session.Get( session_id, globals.SESSION_RUN_STATE );  
   ...
   use EU.BE.Users;
      ...
      disable_everything : Boolean := False;
      data           : AWS.Response.Data;
   begin
      if( logresult.cookie_support = unknown )then -- send test cookie
         data := AWS.Response.URL( Location => EU.Web.Settings.Mefisto_Root & "cookietest/" );
         Log( "making cookie as |" & 
            AWS.Cookie.Create_Cookie_Response(
               key => "tc01", 
               value => "True",
               domain => "localhost",
               max_age => 10000.0,
         ...
         AWS.Cookie.Set( 
            ...
            value   => "True",
            domain  => "",
            max_age => 10000.0, -- AWS.Cookie.No_Max_Age,
            secure  => False ); 
         Log( "TEST COOKIE SET OK" );
         return data;
      elsif( logresult.cookie_support = no )then
         logresult.user.lang := EU.BE.I81N.en; -- jam one or other on.
      elsif(( not logresult.validated ) or ( logresult.new_session )) then
         return logresult.response;
      end if;

      translations := Get_Std_Translations( request, logresult.user );          

      if( logresult.cookie_support = yes )then
         disable_output := Dont_Show_Output( request, logresult.user ); 
         Handle_Language( path, logresult.user, session_id );
      else
         Insert( translations, Assoc( "COOKIES", "Sorry, but you need to have cookies enabled to run the model. Information <a href='http://support.google.com/accounts/bin/answer.py?hl=en&answer=61416'>is available</a> on how to enable cookies in your browser." ));
         Insert( translations, Assoc( "DISABLED", True ));
      end if;
      Insert( translations, Assoc( "FRONT-PAGE", Lookup( "front_page", logresult.user.lang )));
      Insert( translations, Assoc( "HOME-PAGE", Lookup( "home_page", logresult.user.lang )));
      Insert( translations, Assoc( "WELCOME", Lookup( "welcome", logresult.user.lang )));
      Insert( translations, Assoc( "LIGHT-PAGE", Lookup( "light_page", logresult.user.lang )));
      Insert( translations, Assoc( "PARAMETERS-PAGE", Lookup( "parameters_page", logresult.user.lang )));
      Insert( translations, Assoc( "OUTPUT-PAGE", Lookup( "output_page", logresult.user.lang )));
   ...
   begin
      ...
      if( not output.Is_Initialised )then
         ...
         begin
            if( last_run_id > 0 )then
               ...
               Load_Output( local_run_settings, output );
      ...
      Handle_Language( path, logresult.user, session_id );
      translations := Get_Std_Translations( request, logresult.user );
      Insert( translations, Assoc( "OUTPUT-MENU", Get_Output_Menu( summary_page, logresult.user.lang )));
      Insert( translations, Assoc( "OUTPUT-INFORMATION", Lookup( "Output Help", logresult.user.lang )));
         
      path.Delete( 1, 2 ); -- strip "mefisto" and "output_page" from the url
      
      if( Integer( path.Length ) = 0 )then
         Insert( translations, Assoc( "OUTPUT-MENU", Get_Output_Menu( summary_page, logresult.user.lang )));
         return Web_Utils.Build_Input_Page( 
            euws.template_components_path & "output_container", 
            translations );
      end if;
   ...
   type Actions is ( save, run, uprate_all, uprate_shown, reset_all, reset_shown, abort_it, no_action );

   function Extract_Action( action_string : String ) return Actions is
   use globals.BE_HTML_Utils;
      action : Actions := no_action;
   begin
      if( action_string = "save" )then 
         action := save;
      elsif( action_string = "run" )then 
         action := run;
      elsif( action_string = "uprate_all" )then 
         action := uprate_all;
      elsif( action_string = "uprate_shown" )then 
         action := uprate_shown;
      elsif( action_string = "reset_all" )then 
         action := reset_all;
      elsif( action_string = "reset_shown" )then 
         action := reset_shown;
      elsif( action_string = "abort" )then 
         action := abort_it;
      end if;
      Log( "extracted " & action'Img & " from string |" & action_string & "| " );
      return action;
   end Extract_Action;
   ...
   use BE_Base_Model_Types;
      ...
      path               : Unbounded_String_List := Split( URI, '/' );
      action_string      : constant String := AWS.Parameters.Get( params, "submit_action" );
      action             : constant Actions := Extract_Action( action_string );
   ...
   begin
      ...
      lang := logresult.user.lang;
      disable_output := Dont_Show_Output( request, logresult.user ); 
      Log( "Light Page Handler Entered" );
      ...
      translations := Get_Std_Translations( request, logresult.user );
      Log( "Light Page Handler: Got action as " & action'Img & " from string |" & action_string & "| " );
      if action = run or action = save then
         ...
         begin
            ...
            Log( "Light Page Handler: buffer reset OK" );
            EU.BE.Parameter_System_Declarations.Map_To_Parameters( light_pars, param_buffer );          
            Log( "Light Page Handler: parameters mapped OK" );
            Buffer_Session.Set( session_id, globals.SESSION_PARAMETER_BUFFER, param_buffer );
            Log( "Light Page Handler: buffer set to session session id is " & AWS.Session.Image( session_id ));
            Light_Params_Session.Set( session_id, globals.SESSION_LIGHT, light_pars );
            if action = run then
               Log( "Light Page Handler: run entered; num_errors=" & Natural'Image( num_errors ) & " job_is_running " & Boolean'Image( job_is_running ));
               if( not job_is_running ) and num_errors = 0 then
                  ...
                  begin
                     ...
                     Log( "Light Page Handler: creating run; run_settings.Working_Root=" & run_settings.Working_Root & " SEP |" & SEP & "| run_number_str | " & run_number_str );
      ...
      Insert( translations, Assoc( "CHILD-CARE-LABEL", Lookup( "Child Support", lang )));
      ...
      return Web_Utils.Build_Input_Page(
         euws.template_components_path & "input_light_new",
         translations );
   ...
   function Parameter_Page_Callback( request : in AWS.Status.Data ) return AWS.Response.Data is
      ...
      run_state          : runsett.State_Rec := Run_State_Session.Get(
         session_id,
         globals.SESSION_RUN_STATE );
      saved_path_string  : Unbounded_String;
   ...
   begin
      ...
      output_disabled := Dont_Show_Output( request, logresult.user );
      Handle_Language( path, logresult.user, session_id );
      Log( "params are " & AWS.Parameters.URI_Format( params ));
      run_settings := Run_Settings_Session.Get( session_id, globals.SESSION_RUN_SETTINGS );      
      param_buffer := Buffer_Session.Get( session_id, globals.SESSION_PARAMETER_BUFFER );
      param_buffer.Set_Language( logresult.user.lang );     
      extra_translations := Get_Std_Translations( request, logresult.user );
      saved_path_string := TuS( Join( path, '/' ));
      ...
      if( path.Length = 0 )then
         model_menu := be_params.BE_Renderer.Make_Parameter_Menu(       
            GLOBAL_PARAM_SYS, 
            path, 
            prefix, 
            logresult.user.lang, 
            logresult.user );   
         Insert( extra_translations, Assoc( "UPRATE-ALL",  Lookup( "Uprate All", logresult.user.lang )));
         Insert( extra_translations, Assoc( "UPRATE-SHOWN", Lookup( "Uprate Shown", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "RESET-SHOWN", Lookup( "Reset Shown", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "RESET-ALL", Lookup( "Reset All", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "SAVE", Lookup( "Save", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "RUN", Lookup( "Run", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "UPRATE-AMOUNT", Lookup( "Uprate Amount", logresult.user.lang ) ));
         Insert( extra_translations, Assoc( "INFORMATION", Lookup( "Information", logresult.user.lang ) ));
         input_page := be_params.BE_Renderer.Create_Wrapper_Input_Page(
      ...
      when reset_all =>
         ...
         be_params.BE_Parameter_System_IO.Load( 
                buff          => param_buffer,
                global_system => GLOBAL_PARAM_SYS,
                target_system => GLOBAL_PARAM_SYS,
                values        => globals.Get_Default_Params,
                defaults      => globals.Get_Default_Params,
      ...
      when uprate_shown | uprate_all => 
         ...
         begin
            ...
            EU.BE.I81N.Web_Validate(
               ...
               -1_000.0,
               1_000.0 );
            Log( "got m as as |" &  m'Img & " error as " & error'Img  );
            if( error = No_Error )then
               m := 1.0 + ( m / 100.0 );
               if( action = uprate_all )then
      ...
      Insert( extra_translations, Assoc( "PATH", saved_path_string ));
      ...
      Log( "saved_path_string |" & TS( saved_path_string ));
      
      input_page := be_params.BE_Renderer.Create_Inner_Input_Page(
         buff                => param_buffer,
